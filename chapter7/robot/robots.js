...
function construirGrafo(bordes) {
  ...
  function aniadirBorde(desde, hasta) {
    if (grafo[desde] == null) {
      grafo[desde] = [hasta];
    } else {
  ...
  let variable = bordes.map(c => c.split("-"));

  console.log("no se que hay= " , bordes.map(c => c.split("-")));     

 
  
  for (let [desde, hasta] of bordes.map(c => c.split("-"))) {
    aniadirBorde(desde, hasta);
    aniadirBorde(hasta, desde);
  }
  return grafo;
}
